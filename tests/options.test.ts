...
test("normalization rejects invalid enums and clamps every encoder parameter", () => {
  const normalized = normalizeCompressOption({
    preview: { maxSize: 0 },
    resize: { method: "unknown", width: -10, height: 999999, percent: 250, cropWidthRatio: Number.NaN, presetCrop: { paperSize: "unknown" } },
  ...
  assert.equal(normalized.resize.percent, 100);
