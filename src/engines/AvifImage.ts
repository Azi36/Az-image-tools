...
// otherwise every later compression keeps failing.
let encoderReady: Promise<ReturnType<AvifModule["createAvifEncoder"]>> | null = null;

function getEncoder() {
  if (!encoderReady) {
    encoderReady = importCodec<AvifModule>(
      "/codecs/avif/index.browser.mjs",
    ).then((module) => module.createAvifEncoder("client")).catch((error) => {
      encoderReady = null;
      throw error;
    });
  }
