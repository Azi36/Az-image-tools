...
export abstract class ImageBase {
  ...
  getOutputDimension(): Dimension {
    ...
    // Scale both edges by a percentage of the original size
    if (method === "scalePercent") {
      ...
      return {
        x: 0,
        y: 0,
        width: Math.max(1, Math.ceil((this.info.width * percent) / 100)),
        height: Math.max(1, Math.ceil((this.info.height * percent) / 100)),
      };
    }
