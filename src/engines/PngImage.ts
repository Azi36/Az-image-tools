...
type PngCodecs = {
  quantize: ReturnType<ImageQuantModule["createImagequantQuantizer"]>;
  optimize: ReturnType<OxiPngModule["createOxipngOptimizer"]>;
};

// Lazy with retry: a failed load (e.g. flaky network) must not be
// cached forever, otherwise every later compression keeps failing.
let codecsReady: Promise<PngCodecs> | null = null;
...
export class PngImage extends ImageBase {
  static async encode(
    ...
    const { quantize, optimize } = await getCodecs();
